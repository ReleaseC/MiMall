    ...
    <div class="wrapper">
      <div class="container">
        ...
        <div class="item-pay">
          ...
          <div class="pay-way">
            <p>支付平台</p>
            <div class="pay pay-ali" :class="{'payType':payType==1}" @click="paySubmit(1)"></div>
            <div class="pay pay-wechat" :class="{'payType':payType==2}" @click="paySubmit(2)"></div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods:{
    ...
    paySubmit(payType){
      if(payType == 1){
        window.open('/#/order/alipay?orderId='+777,'_blank');
      }else{
        this.showPay = true
      }
      ORCode.toDataURL('').then(url => {
        console.log(url)
      })
      .catch(err => {
        console.error(err)
